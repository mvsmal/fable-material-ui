...
```fsharp
Fable.MaterialUI.Core.select (props : seq<IHTMLProp>) (children : seq<ReactElement>) : ReactElement
```
...
| <span class="prop-name">SelectProp.Input</span> | <span class="prop-type">ReactElement</span> | <span class="prop-default">&lt;Input /></span> | An `Input` element; does not have to be a material-ui specific `Input`. |
